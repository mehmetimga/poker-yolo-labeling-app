...
async def get_annotations(image_id: int, _: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def save_annotations_endpoint(
    image_id: int, body: AnnotationsBulkSave, _: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
    try:
